...
export function Input({ label, ...props }: InputProps) {
  return (
    <div class="mb-6">
      {label && (
        <label>
          <p class="block antialiased font-sans text-sm leading-normal mb-2 font-medium text-gray-900">
            {label}
          </p>
        </label>
      )}
      <div class="relative w-full min-w-[200px] h-11">
        {props.type === "password" && (
          <div class="grid cursor-pointer place-items-center absolute text-blue-gray-500 top-2/4 right-3 -translate-y-2/4 w-6 h-6">
            <i>
              <svg
        ...
        <input
          class="peer h-full bg-transparent text-blue-gray-700 font-sans font-normal outline-0 focus:outline-0 disabled:bg-blue-gray-50 disabled:border-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 border focus:placeholder:opacity-100 text-sm px-3 py-3 rounded-md border-blue-gray-200 focus:border-blue-900 w-full placeholder:opacity-100"
          {...props}
        />
      </div>
    </div>
